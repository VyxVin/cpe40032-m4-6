...
function PlayState:moves(tile1, tile2, invalidSwap)

    self.board:moves(tile1, tile2)

    if invalidSwap then
        Timer.tween(0.1,{
            [tile1] = {x = tile2.x, y = tile2.y},
            [tile2] = {x = tile1.x, y = tile2.y}
        })
...
function PlayState:calculateMatches()
    ...
    local matches = calculateMatches or self.board:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            -- gets 1 extra second per #match if match
        ...
        if not self.board:anyMoves() then
            self.noMatches = true
            gSounds['error']:play()
            Timer.tween(0.5, {
            ...
            :finish(function()
                Timer.after(2, function()
                    ...
                    :finish(function()
                        while not self.board:anyMoves() do
